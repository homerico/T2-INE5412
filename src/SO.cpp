...
void SO::createProcess(std::vector<int> entries) {
    // Cria processos com pid gerados sequencialmente
    unsigned pid = this->processes.size();
    this->processes.push_front(new Process(pid, entries));
...
int SO::getAddressFromPageTable(unsigned pid, int virtualAddr) {
    ...
    PageTable* pt = pMem->pt;
    PageTable::PTEntry* ptEntry = pt->getEntry(virtualAddr);
    
    if (!ptEntry->present) {
        // Page Table Miss
        this->pageMissCount++;
        std::tuple<int,int> memTuple = this->prAlgorithim->findMemory(virtualAddr);
        pt->removeEntry(std::get<0>(memTuple));
