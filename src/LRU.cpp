...
std::tuple<int,int> LRU::findMemory(int nextVirtualAddrAccess) {
    if(this->accessList.size() < this->accessListMaxSize) {
        ...
        return std::tuple<int,int>{nextVirtualAddrAccess, this->accessList.size()};
    }
    ...
    return std::tuple<int,int>{nextVirtualAddrAccess,this->memTranslate[nextVirtualAddrAccess]};
}
